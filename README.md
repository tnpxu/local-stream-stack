...
| Connection setup timed out or Failed to resolve | The Kafka broker was advertising its internal K8s address to external clients. After connecting to the bootstrap, the client would try to connect to this unreachable address and fail.                                                         | The `kafka-kraft-cluster.yaml` now has a configuration.brokers block in the external listener. `This forces the broker to advertise 127.0.0.1:9094`, directing all client traffic back through the reliable port-forward tunnel.                                  |
